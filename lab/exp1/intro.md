...
Create a default Unix user account: riya 
New password:  
Retype new password:  
...
```txt
jriya@0420041971:/mnt/c/WINDOWS/system32$ sudo apt install docker.io  
sudo systemdctl start docker  
[sudo] password for jriya:  
Sorry, try again.  
[sudo] password for jriya:  
...
docker pullnginx  
```txt
jriya@0420041971:/mnt/c/WINDOWS/system32$ docker pull nginx  
...
```txt
jriya@0420041971:/mnt/c/WINDOWS/system32$ docker run -d -p 8080:80 --name nginx-container nginx  
4edbc7838396ad3d0d0a53f04bfo1a1bfbdc06fbdb66c968043aaf90a1afc0fc  
jriya@0420041971:/mnt/c/WINDOWS/system32$ docker ps  
...
jriya@0420041971:/mnt/c/WINDOWS/system32~$
```

Step 5: Verify Nginx Container curl localhost:8080  
```html
jriya@0420041971:/mnt/c/WINDOWS/system32$ curl localhost:8080
<!DOCTYPE html>
<html>
<head>
...
- **Box download failure** → Check firewall settings
- **SSH timeout** → Run `vagrant reload`

## Docker Issues (WSL)

- **Permission denied** → Ensure the user is added to the docker group  
  ```bash
  sudo usermod -aG docker $USER
exp1
